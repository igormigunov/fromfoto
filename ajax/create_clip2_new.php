...
if($_POST){
	...
	if(preg_match("/\@mail\.ru|\@bk\.ru|\@inbox\.ru|\@list\.ru/i",$_POST['email'])){
		//$errors[] = "К сожалению, мы не сможем выслать готовое видео на почтовые ящики @mail.ru, @bk.ru, @inbox.ru, @list.ru, пожалуйста, выберите любой другой";
	}
	}
...
if(!$errors){
		...
		foreach($_POST['imgs'] as $k=>$v){
			...
			if(isset($_POST['coords'][$k])){
				...
				$arImages[] = array('file'=>$_SERVER['DOCUMENT_ROOT'].$new_path,
									'x1'=>$_POST['coords'][$k][0],
									'x2'=>$_POST['coords'][$k][1],
									'y1'=>$_POST['coords'][$k][2],
									'y2'=>$_POST['coords'][$k][3],
		...
		//tolik
		$APPLICATION->SaveFileContent($uploaddir.'data.txt', serialize($arImages));
		}
