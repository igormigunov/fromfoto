...
{ 
  ...
  function __construct($token,$config,$logger,$root_dir = '')
  {
    $this->auth = $auth;
    $this->config = $config; 
    $this->token = $token;
  ...
  {
	$base_fdr = ($_REQUEST['make_dir']) ? $_REQUEST['make_dir'] : 'zakaz';
	if (!mkdir('/home/admin/'.$base_fdr.$folder, 0755, true)){
		$this->logger->logg("не удалось создать папку $folder...");
		return false; 
	}
  ...
  {
	$base_fdr = ($_REQUEST['make_dir']) ? $_REQUEST['make_dir'] : 'zakaz';  
	if (!copy($_SERVER['DOCUMENT_ROOT'].$local_src,'/home/admin/'.$base_fdr.$file)){
		$this->logger->logg("не удалось скопировать $file...");
		return false; 
	}
	usleep(1000*200);
