	...
	<?foreach($arResult["SECTION"] as $arSection):?>
		...
		<?foreach($arResult["ELEMENT_SECTION"][$arSection["ID"]] as $arElement):?>
			...
			<div class="tab-item<?if($cnt%2 == 1):?> margin-right-0<?endif;?>">
				<div class="tab-item-img" _section_id="<?=$arSection["ID"];?>" _id="<?=$arItem["ID"]; ?>">
					<? if($arParams['PICT_LOGICK']): ?>
						...
						<?
							$newWidth = 666;
							$newHeight = $arSection["ID"] == '186' ? 666:376;
							$renderImage = array();
					...
					<div class="like">
						<div class="price-box">
							<?if($arItem['PROPERTIES']['COST']['VALUE']):?>
								<?=$arItem['PROPERTIES']['COST']['VALUE'];?> <?=$arResult["CURRENCY"][$arItem['PROPERTIES']['CURRENCY']['VALUE_ENUM_ID']]?> <div class="like_span">Без предоплаты</div>
							<?else:?>
								<?=GetMessage('FF_FREE')?>
							<?endif;?>
						</div>
