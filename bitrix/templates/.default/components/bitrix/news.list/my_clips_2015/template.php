...
<div class="add_email_block">
	...
	<div id="phrase">
		Ваш текущий e-mail - <span style="color: red"><?=$arItem['PROPERTIES']['USER_EMAIL']['VALUE'];?></span><br />
		Поменяйте e-mail, если ошиблись. ссылку на клип мы отправим сюда:<br />
	</div>
	<br />
	<div class="clips_email mail">
		<input type="text" _id="<?=$arItem['ID'];?>" class="email_feed">
	...
	$('.add_email').click(function(e){
		...
		$.post("<?=SITE_DIR?>ajax/create_person_by_email.php", { email: $('.email_feed').val(), arItemId: $('.email_feed').attr('_id') }, function(data){
			if(data != '1'){
				...
				$('#phrase').html('');
				$('.add_email_block .email_feed').val(_email);
				isset_email = false;
			}	
		});
	});
