...
$out_summ = number_format(preg_replace("/\s/","",$ar_clip['PROPERTY_COST_VALUE']), 2, '.', '');
        ...
        <p class="text-done"><?=GetMessage("S_TITLE2")?> <?//=number_format(preg_replace("/\s/","",$ar_clip['PROPERTY_COST_VALUE']), 0, '', '')?> <?//=Get_currency($ar_clip['PROPERTY_CURRENCY_ENUM_ID'])?></p>
        <? if($ar_clip['PROPERTY_FREE_PRICE_VALUE']): ?>
...
<input type="submit" name="m_process" class="btn btn-primary" value="<?=GetMessage("S_PAY");?>" />
       ...
       <p class="text-done"><?=GetMessage("S_TITLE5")?></p>
		<p style="margin: auto 25%; padding: 7px; border-radius: 3px;"><?=GetMessage("S_TITLE4")?></p>
       ...
       <br />-->
       <br />
       <a class="btn_go create-link" href="/fljvrFG/" style="font-size: 12px; padding: 7px; height: auto; text-transform: uppercase;"><?=GetMessage("S_BACK")?></a>
       <br />
       <br />
		<? else:?>
