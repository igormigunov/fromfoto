	...
	<div class="img_prev_upl">
		<span class="preview">            	
                ...
                {% if (file.thumbnailUrl) { %}
                    ...
                    <img href="{%=file.url%}" id="img_n-{%=i%}" real_width="{%=file.real_width%}" real_height="{%=file.real_height%}" file_name="{%=file.name%}" uniq_name="{%=file.name%}-{%=i%}" src="{%=file.url%}">
                {% } %}
...
function check_video_size(){
	...
	for(i = 0; i < count_pictures ; i++){
		...
		if(elem.length){
			if(coords[i]){
				if((coords[i][6] != parseInt(elem.attr('clip_width'))) || (coords[i][7] != parseInt(elem.attr('clip_height')))){
					$('.clip_num_'+i).remove();
...
var controls = {
	...
	$('.check_before_submit').click(function(e){
		...
		$.post("<?=SITE_DIR?>ajax/check_before_create.php", {free_period:free_period,SECTION_ID:'<?=$arResult['PROPERTIES']['PHOTO']['VALUE']; ?>', coords:coords,no_logo:no_logo,imgs:imgs,texts:texts, email:$("#email_feed").val(), email_vk:$("#email_vk").val(), name:$("#name_feed").val(), mp3:$('#path_to_mp3').attr('path'), vk:1, video_id:'<?=$_REQUEST['video_id'];?>', maket:'<?=trim($arResult['PROPERTIES']['NAME_AE']['VALUE']);?>', user_logo:user_logo, user_width:user_width, user_logo_src:user_logo_src, ph_user_logo:ph_user_logo, ph_user_width:ph_user_width, ph_user_logo_src:ph_user_logo_src, section_text_id:"<?=$arResult['PROPERTIES']['TEXTS']['VALUE']; ?>"}, function(data){

			if(data == 1){
				$('#best_order_clip_btn').trigger('click');
			}else{
	...
	$("#order_clip_btn, #best_order_clip_btn, .a_order_clip_btn_vk, .a_order_clip_btn_ok, .a_order_clip_btn_fb").click(function(e){
	...
	function show_photo(numbr){
		...
		if($(".preview img",tr_need).hasClass('video_pleer')){
			...
			$('.files .img_prev_upl img').each(function(index, element) {
            	if($(this).attr('uniq_name') == $('img',$(".table.table-striped .img_prev_upl").eq(cur_picture)).attr('uniq_name')){
