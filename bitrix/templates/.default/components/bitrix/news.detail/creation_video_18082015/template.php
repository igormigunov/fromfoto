...
<?
$tags = array();
if($arResult['DISPLAY_PROPERTIES']['TAGS']['VALUE']){
	$tags = explode(",", $arResult['DISPLAY_PROPERTIES']['TAGS']['VALUE']);
}
?>
<div class="page-title" style="text-align: center;">
<? if($tags): ?>
	<? for($i = 0; $i < sizeof($tags); $i++): ?>
		<span style="<? if($i == 0): ?>padding-left: 0px; <? endif; ?> font-size: 12px;">
			<?=trim($tags[$i]); ?>
		</span>
	<? endfor; ?>
<? else: ?>
	<span style="padding-left: 0px;">шаблон <?=$arResult["NAME"]?></span>
<? endif; ?>
</div>
...
<tr>
		...
		<td class="left-redactor redactor-navigation">
			<?
						/*<? if($arResult['LEFT_PAGE']): ?>
				<? if(!is_mobile()): ?>
					<a href="<?=$arResult['LEFT_PAGE']['URL'];?>">
						
							if($arResult['LEFT_PAGE']['PREVIEW_PICTURE']){
								$renderImage = CFile::ResizeImageGet($arResult['LEFT_PAGE']['PREVIEW_PICTURE'], Array("width" => $newWidth, "height" => $newHeight), BX_RESIZE_IMAGE_PROPORTIONAL);
							}
							if(!file_exists($_SERVER['DOCUMENT_ROOT'].$renderImage['src'])){
								$renderImage['src'] = "/images/tab-item.jpg";
							}
						
							<div class="preview_clip_img">
								<img src="<?=$renderImage['src']; ?>" />
							</div>
						
						<div class="preview_clip_str left_clip_str">
							<img class=""  src="/images/left_redactor_str.png" />
						</div>
						<img class="preview_clip_img_src"  src="/images/left_redactor_str.png" />
					</a>
				<? else: ?>
					<a href="<?=$arResult['RIGHT_PAGE']['URL'];?>">
						<img class="preview_clip_img_src" style="height: 50px; margin-right: 5px;" class=""  src="/images/left_redactor_str.png" />
					</a>
				<? endif; ?>
			<? endif; ?>
			*/
	...
	<td class="template-video">
		<?=$arResult['DISPLAY_PROPERTIES']['PREV_VIMEO2']['~VALUE'];?>
	</td>
	
		<td class="right-redactor redactor-navigation">
			<?/*<? if($arResult['RIGHT_PAGE']): ?>
				<? if(!is_mobile()): ?>
					
						
					<a href="<?=$arResult['RIGHT_PAGE']['URL'];?>">
						
							if($arResult['RIGHT_PAGE']['PREVIEW_PICTURE']){
								$renderImage = CFile::ResizeImageGet($arResult['RIGHT_PAGE']['PREVIEW_PICTURE'], Array("width" => $newWidth, "height" => $newHeight), BX_RESIZE_IMAGE_PROPORTIONAL);
							}
							if(!file_exists($_SERVER['DOCUMENT_ROOT'].$renderImage['src'])){
								$renderImage['src'] = "/images/tab-item.jpg";
							}
						...
						<div class="preview_clip_str right_clip_str">
							<img class="" src="/images/right_redactor_str.png" />
						</div>
						<img class="preview_clip_img_src" src="/images/right_redactor_str.png" />
					</a>
				<? else: ?>
					<a class="" href="<?=$arResult['RIGHT_PAGE']['URL'];?>">
						<img style="height: 50px; margin-left: 5px;" class="" src="/images/right_redactor_str.png" />
					</a>
				<? endif; ?>
			<? endif; ?>
			*/
...
<?endif;?>
		...
		<? else: ?>
			...
			<? else: ?>
			<input <? if($_SESSION["user_clip_data"] && 1==0):?> style="display: none; "<? endif; ?> type="submit" onclick="yaCounter25315490.reachGoal('zakaz-video');" class="button-slide auth_clip_please" value="<?=GetMessage("FF_BUT_SEND")?>">
			<? endif; ?>
...
<script>
	...
	function send_clip(_make_dir){		
		...
		$(".auth_clip_please").hide();
		$("#answer").html('Пожалуйста, подождите...');
		...
		$.post("<?=SITE_DIR?>ajax/create_clip.php", 
			...
			if(data == 1){
				...
				{	SECTION_ID:'<?=$arResult['PROPERTIES']['PHOTO']['VALUE']; ?>',
					...
					{},
					function(data){
						window.location = '/fljvrFG/prev.php?PRODUCT_ID='+data;
					});
				});
			}else{
				$("#answer").html(data);
