...
<div <? if($_SESSION["user_clip_data"]):?> style="display: none; "<? endif; ?> class="template-video">
	<?=$arResult['DISPLAY_PROPERTIES']['PREV_VIMEO2']['~VALUE'];?>
</div>

      <div class="template-step margin" <? if($_SESSION["user_clip_data"]):?> style="display: none; "<? endif; ?>>
        <span>ШАГ 1</span> 
	  ...
	  <form id="fileupload" action="/ajax/upload.php" method="POST" enctype="multipart/form-data" <? if($_SESSION["user_clip_data"]):?> style="display: none; "<? endif; ?>>
	...
	<div class="step_second">
      <div class="template-step" <? if($_SESSION["user_clip_data"]):?> style="display: none; "<? endif; ?>>
        <span>ШАГ 2</span> <?=GetMessage("FF_ST2_TITLE")?>
      </div>
      <div class="upload-wrap" <? if($_SESSION["user_clip_data"]):?> style="display: none; "<? endif; ?>>
	...
	<?foreach($arResult["TEXTS"] as $k=>$v):?>
        ...
        <div class="template-text">
          <div class="template-text-container">
            ...
            <textarea class="text-field text_inp" size_value="<?=$v['PROPERTY_COUNT_SYMBOLS_VALUE']; ?>" placeholder="Напишите сюда ваш текст до <?=$v['PROPERTY_COUNT_SYMBOLS_VALUE']; ?> символов"><?=substr($arTexts[$k], 0, intval($v['PROPERTY_COUNT_SYMBOLS_VALUE']));?></textarea>
          </div>
...
<?endif;?>
        ...
        <div class="mail">
          ...
          <input style="margin-top: -23px;" id="email_feed" type="text" placeholder="почта" value="<?=$cookie_login;?>">
        </div>
        <div class="slide-title-FOC" style="margin: 30px 0">
          <span><?=GetMessage("FF_MESS1")?></span> 
        </div>
		<? endif; ?>
		<div style="text-align: center; font-weight: bold; color: #5d7395;" class="hide_after_order" id="answer"><? if($_SESSION["user_clip_data"]):?><?=GetMessage("S_PROCESSING")?><? endif; ?></div>
		<input type="submit" class="a_order_clip_btn_vk" style="display: none;" value="Отправить" />
        <input <? if($_SESSION["user_clip_data"]):?> style="display: none; "<? endif; ?> onclick="yaCounter25315490.reachGoal('zakaz-video');" type="submit" class="button-slide check_before_submit order_clip_please" value="<?=GetMessage("FF_BUT_SEND")?>">
...
<div class="popup" style="display: none;">
	<div class="popup-container with_border" style="margin: -160px auto 0; top: 50%;">
		...
		<?$APPLICATION->IncludeComponent("bitrix:system.auth.form","soc_create",Array(
		...
		<span class="ready-klip normal big-kl" style="text-decoration: uppercase; margin: 40px 0 20px;" >
			ИЛИ оплатите 100р., ЕСЛИ НЕ ХОТИТЕ ДЕЛАТЬ РЕПОСТ
		</span>
		<a class="button-slide under-content-title pay_clip_vk" style="text-decoration: uppercase; margin-bottom: 10px; width: 256px;" href="#">оплатить<a>
	</div>
</div>
		...
		$.post("<?=SITE_DIR?>ajax/create_clip.php", 
			...
			if(data == 1){
				...
				{	SECTION_ID:'<?=$arResult['PROPERTIES']['PHOTO']['VALUE']; ?>',
					...
					maket:'<?=trim($arResult['PROPERTIES']['NAME_AE']['VALUE']);?>', 
					section_text_id:"<?=$arResult['PROPERTIES']['TEXTS']['VALUE']; ?>",
					make_dir : <? if($_SESSION['paid']):?>'wait_zakaz'<? else: ?>_make_dir<? endif; ?>
				}, function(data){
					<? if($_SESSION['paid']):?>
					...
					<? else: ?>
						setTimeout(function(){window.location = '/fljvrFG/?check=1';}, 5*1000);	
					<? endif; ?>
				});
			}else{
				$("#answer").html(data);
