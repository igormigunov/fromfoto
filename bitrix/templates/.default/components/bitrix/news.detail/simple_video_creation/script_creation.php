...
var controls = {
	...
	$("#order_clip_btn, .a_order_clip_btn").click(function(e){
		...
		if($('#logo_viz .logo_img').length){
			user_logo = 1;
			user_width = $('#logo_viz').attr('logo_width');
			user_logo_src = $('#logo_viz .logo_img').attr('src');
		}
		...
		$.post("<?=SITE_DIR?>ajax/create_clip.php", {free_period:free_period,SECTION_ID:'<?=$arResult['PROPERTIES']['PHOTO']['VALUE']; ?>', coords:coords,no_logo:no_logo,imgs:imgs,texts:texts, email:$("#email_feed").val(), name:$("#name_feed").val(), mp3:$('#path_to_mp3').attr('path'), video_id:'<?=$_REQUEST['video_id'];?>', maket:'<?=trim($arResult['PROPERTIES']['NAME_AE']['VALUE']);?>', simple:'simple', user_logo:user_logo, user_width:user_width, user_logo_src:user_logo_src, ph_user_logo:ph_user_logo, ph_user_width:ph_user_width, ph_user_logo_src:ph_user_logo_src}, function(data){
