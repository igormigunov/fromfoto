...
$('#no_logo').change(function(){
	if($(this).is(':checked')){
		$('.cost_m').html($('.cost_m').attr('plus_cost'));
	}else{
		$('.cost_m').html($('.cost_m').attr('real_cost'));
	}
});
...
var controls = {
	...
	$("#order_clip_btn").click(function(e){
		...
		$.post("<?=SITE_DIR?>ajax/create_clip.php", {coords:coords,no_logo:no_logo,imgs:imgs,texts:texts, email:$("#email_feed").val(), name:$("#name_feed").val(), mp3:$('#path_to_mp3').attr('path'), video_id:'<?=$_REQUEST['video_id'];?>', maket:'<?=trim($arResult['PROPERTIES']['NAME_AE']['VALUE']);?>'}, function(data){
