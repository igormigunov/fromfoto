...
if($('.logo_photograph #add_logo').length){
	...
	$('.next_prev_img .prev_img img').click(function(e) {
        if(num_next_prev>0){
			...
			num_next_prev--;
			pause_p();
			$('.img_prev_upl img').not('.check').each(function(index, element) {
	...
	$('.next_prev_img .next_img img').click(function(e) {
        if(num_next_prev<(count_pictures-1) && ($('.img_prev_upl img').not('.check').length-1) > num_next_prev){
			clearTimeout(select_first_photo);
			num_next_prev++;
			pause_p();
			$('.img_prev_upl img').not('.check').each(function(index, element) {
	...
	$('.img_prev_upl img.check').click(function(e) {
		$('img.shprew', $(this).closest('.img_prev_upl')).trigger('click');
	});

	$('.img_prev_upl img').not('.check').click(function(e) {
...
//запись текстов
	...
	$(function(){
		...
		$("#fileupload .files1").sortable({
	    	...
	    	start: function(event, ui) {
	    		$('.img_prev_upl img').not('.check').removeClass('ui-selected');
			},
	    	...
	    	stop: function(event, ui) {
				...
				$("#fileupload .files .img_prev_upl .preview img").not('.check').each(function(){
...
function img_nums_res(){
	...
	$("#fileupload .img_prev_upl .preview img").not('.check').each(function(index, element){
		$(this).attr('id', 'img_n-'+index);
	});
}

$('div.img_prev_upl').click(function(){
	$('.check', $(this)).show(500);
})
</script>
...
var timer_change_count_text_photo = setTimeout(function change_count_text_photo() {
	...
	difference = parseInt(count_pictures) - parseInt($('.img_prev_upl .preview img').not('.check').length);
	if(difference > 0){
	...
	if(parseInt($('.img_prev_upl .preview img').not('.check').length)<=0 || !$('.img_area .show_img:visible').length){
...
var controls = {
	...
	function show_photo(numbr){
		...
		if(alr_curr != cur_picture){
			...
			$('.files .img_prev_upl img').not('.check').removeClass("ui-selected");
		}
		...
		if($(".preview img",tr_need).not('.check').hasClass('video_pleer')){
			...
			$('.files .img_prev_upl img').not('.check').each(function(index, element) {
