...
function show_vertical_size(){
	...
	$(".table-striped .img_prev_upl .preview img").each(function(index, element){
		if(!$(this).hasClass('video_pleer') && coords[index]){
			...
			if((n_new_height/n_work_area_height) > 1.4 && !$('.clip_num_'+index+' img').length){
...
var controls = {
	...
	$("#order_clip_btn, .a_order_clip_btn").click(function(e){
		...
		<? if(!$USER->IsAuthorized()): ?>
			$.post("<?=SITE_DIR?>ajax/prepare_auth.php", {free_period:free_period,SECTION_ID:'<?=$arResult['PROPERTIES']['PHOTO']['VALUE']; ?>', coords:coords,no_logo:no_logo,imgs:imgs,texts:texts, email:$("#email_feed").val(), name:$("#name_feed").val(), mp3:$('#path_to_mp3').attr('path'), video_id:'<?=$_REQUEST['video_id'];?>', maket:'<?=trim($arResult['PROPERTIES']['NAME_AE']['VALUE']);?>', user_logo:user_logo, user_width:user_width, user_logo_src:user_logo_src, ph_user_logo:ph_user_logo, ph_user_width:ph_user_width, ph_user_logo_src:ph_user_logo_src, section_text_id:"<?=$arResult['PROPERTIES']['TEXTS']['VALUE']; ?>"}, function(data){
				if(data == 1){
					var _css = {
						width : "744px",
						height : "460px"
					};
					$('#preview_popup').css(_css);
					$('#preview_popup').css('margin-top',"-230px");
					$('#preview_popup').css('margin-left',"-372px");

					$('.preview_popup .text_popup').css(_css);
					
					
					$('#preview_popup_full, #preview_popup, #preview_popup .preview_popup').hide();
					$('#preview_popup_full, #preview_popup, #preview_popup #auth_social').show();
				}else{
					$("#answer").html(data);
					$("#order_clip_btn").show();
				}
	...
	function show_photo(numbr){
		...
		if(!$('.clip_num_'+cur_picture).length){
			...
			$('.img_clips.clip_num_'+cur_picture+' img').cropbox({
				...
				//console.log('crop window: ' + data.cropX);1
    		});
